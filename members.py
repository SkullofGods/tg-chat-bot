...
def plain_name(user_id: int, row: dict | None = None) -> str:
    ...
    name = (row.get("nickname") or row.get("full_name") or row.get("username") or "").strip()
...
async def random_member(chat_id: int, exclude: Iterable[int] = (), active_days: int | None = None) -> int | None:
    ...
    if active_days:
        ...
        active = [c for c in candidates if (parse_utc(c["last_active"]) or border) > border]
