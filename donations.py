...
async def announce(donation: dict):
    text = texts.DONATION_ANNOUNCE.format(name=members.mention(donation["user_id"]), amount=fmt_num(donation["amount"]))
    if donation["message"]:
...
async def decide(donation_id: int, approve: bool) -> str:
    ...
    if not db.decide_donation(donation_id, "confirmed" if approve else "rejected"):
        return "Этот донат уже проверен"
    if not approve:
        return "Отклонено"
    await announce(donation)
    return "Объявил в беседе ❤️"
...
async def add_confirmed(chat_id: int, user_id: int, amount: int, message: str):
    ...
    donation_id = db.add_donation(chat_id, user_id, amount, clean_message(message), status="confirmed")
